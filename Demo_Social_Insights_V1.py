				...
				if hcpname is not None and hcpname!=" ":
					...
					with st.spinner("Ploting the Graphs"):
						...
						with col6:
							...
							st.pyplot(plt)
							highlighted_text = f"<p style='color: black;'><span style='color: blue;'font-weight: bold';>{hcpname} </span> is discussing <span style='color: blue;'>{val.index[0]} </span>as a key area of focus with <span style='color: green;'>{round(val[0],2)}%</span> engagement rate.</p>"
